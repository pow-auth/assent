defmodule Assent.HTTPAdapter.MintTest do
  ...
  describe "request/4" do
    test "handles SSL" do
      ...
      assert CaptureIO.capture_io(:stderr, fn ->
               assert {:ok, %HTTPResponse{status: 200, body: "HTTP/1.1"}} =
                        Mint.request(:get, TestServer.url(), nil, [], mint_opts)
             end) =~
               "Assent.HTTPAdapter.Mint is deprecated, consider use Assent.HTTPAdapter.Finch instead"
    end
    ...
    test "handles SSL with bad certificate" do
      ...
      assert CaptureIO.capture_io(:stderr, fn ->
               assert {:error,
                       %TransportError{reason: {:tls_alert, {:handshake_failure, _error}}}} =
                        Mint.request(:get, bad_host_url, nil, [], mint_opts)
             end) =~
               "Assent.HTTPAdapter.Mint is deprecated, consider use Assent.HTTPAdapter.Finch instead"
    end
    ...
    test "handles query in URL" do
      ...
      assert CaptureIO.capture_io(:stderr, fn ->
               assert {:ok, %HTTPResponse{status: 200}} =
                        Mint.request(:get, TestServer.url("/get?a=1"), nil, [])
             end) =~
               "Assent.HTTPAdapter.Mint is deprecated, consider use Assent.HTTPAdapter.Finch instead"
    end
    ...
    test "handles POST" do
      ...
      assert CaptureIO.capture_io(:stderr, fn ->
               assert {:ok, %HTTPResponse{status: 200}} =
                        Mint.request(:post, TestServer.url("/post"), "a=1&b=2", [
